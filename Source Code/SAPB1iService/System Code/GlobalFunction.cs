...
namespace FTSISAPB1iService
{
    class GlobalFunction
    {
        ...
        public static string getUSeriesbyGroupCode(string tableName, string strGroupCode, string strColumnName)
        {
            string ObjectCode = string.Empty;

            SAPbobsCOM.Recordset oRecordset =
                (SAPbobsCOM.Recordset)GlobalVariable.oCompany.GetBusinessObject(SAPbobsCOM.BoObjectTypes.BoRecordset);

            string query = $"SELECT \"U_Series\" FROM {tableName} WHERE \"{strColumnName}\" = '{strGroupCode}'";

            oRecordset.DoQuery(query);

            if (!oRecordset.EoF) 
            {
                ObjectCode = oRecordset.Fields.Item("U_Series").Value.ToString();
            }
